        ...
        <Reveal key={s.label} delay={i * 0.06}>
          <div
            className={cn(
              ...
              light ? 'border-white/10 bg-white/5' : 'border-surface-muted bg-white shadow-soft',
            )}
          >
    ...
    <ol className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
      {steps.map((s, i) => (
        <Reveal as="li" key={s.title} delay={i * 0.06} className="relative">
          <div className="text-5xl font-extrabold text-surface-muted">{String(i + 1).padStart(2, '0')}</div>
...
export function CTABand({ title, body, primary, secondary, tone = 'ink' }) {
  const dark = tone === 'ink'
  return (
    <div className={cn('relative overflow-hidden rounded-4xl px-6 py-12 sm:px-12', dark ? 'bg-ink text-white' : 'bg-brand-50')}>
      {dark && <div className="pointer-events-none absolute inset-0 bg-brand-radial" />}
      <div className="relative flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
        <div className="max-w-2xl">
          <h2 className={cn('text-2xl font-extrabold sm:text-3xl', dark ? 'text-white' : 'text-ink')}>{title}</h2>
          {body && <p className={cn('mt-2', dark ? 'text-white/70' : 'text-ink-muted')}>{body}</p>}
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          {secondary && (
            <Button
              ...
              size="lg"
              className={dark ? 'border-white/30 bg-white/10 text-white hover:bg-white/20' : ''}
            >
              {secondary.label}
            </Button>
          )}
